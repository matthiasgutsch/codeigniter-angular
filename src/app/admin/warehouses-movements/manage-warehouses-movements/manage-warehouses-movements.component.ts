...
import { PARAM_CHECKINS_PATH, PARAM_WAREHOUSE_MOVEMENTS_PATH, PARAM_WORKS_PATH, STATUS_PRODUCTS } from '../../constants/constants';
...
@Component({
  selector: 'app-manage-warehouses-movements',
  templateUrl: './manage-warehouses-movements.component.html'
})
export class ManageWarehousesMovementsComponent implements OnInit {
  ...
  getRequestParams(searchTitle, categoryTitle, page, pageSize): any {
    ...
    let path = PARAM_WAREHOUSE_MOVEMENTS_PATH;
    const params = {};
    let adder = '?';
    if (page) {
      params[`page`] = page - 1;
