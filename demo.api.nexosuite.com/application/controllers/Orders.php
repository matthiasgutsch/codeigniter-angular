...
public function get_order_by_quotes_id($user_id)
	{	
		...
		$items = $this->orders_model->get_order_by_quotes_id($user_id);
	...
	{	
		...
		$items = $this->orders_model->get_orders_by_client($id);
	...
	public function count($id)
	{
		...
		$token = $this->input->get_request_header('Authorization');
			...
			$posts = $this->orders_model->count_orders($id);
	...
	public function count_total($id)
	{
		...
		$posts = $this->orders_model->count_total_orders($id);
...
public function orders_by_appointment_id($user_id)
	{	
		...
		$items = $this->orders_model->get_orders_by_appointment_id($user_id);
...
public function orders_by_appointments($user_id)
	{	
		...
		$items = $this->orders_model->get_orders_by_appointments($user_id);
	...
	public function index()
	{
		...
		$posts = array();
		if($isValidToken) {
			$blogs = $this->orders_model->get_orders();
...
public function list_user($user_id)
	{
		...
		$size = $this->orders_model->count_orders_search($category_name, $category_description, $user_id);
		...
		$isValidToken = $this->api_model->checkToken($token, $user_id);
			...
			$blogs = $this->orders_model->get_list($user_id, $_start, $_limit, $_sort, $category_name, $category_description);
...
public function user($user_id)
	{
		...
		$posts = array();
		if($isValidToken) {
			$blogs = $this->orders_model->get_orders($user_id);
			foreach($blogs as $blog) {
				$posts[] = array(
					'id' => $blog->id,
					'title' => $blog->title,
	...
	public function id($appointment_id, $user_id)
	{
		...
		if($isValidToken) {
			...
			$blog = $this->orders_model->get_id($appointment_id);
...
public function skills($id, $user_id)
	{
		...
		if($isValidToken) {
			...
			$blog = $this->orders_model->get_id($id);
...
public function create($user_id)
	{
		...
		if($isValidToken) {
			...
			if( ! $isUploadError) {
				...
				$id = $this->orders_model->create($blogData);
...
public function update_skills($id, $user_id)
	{
		...
		if($isValidToken) {
			...
			if( ! $isUploadError) {
				...
				$this->orders_model->update($id, $blogData);
