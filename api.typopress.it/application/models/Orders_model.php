...
public function get_order_by_quotes_id($id = null)
	...
	public function get_orders_by_client($id = null)
	{
        if ($id != null) {
            $this->db->select('*');
            $this->db->from('orders');
...
public function get_list($user_id, $_start, $_limit, $_sort, $category_name, $category_description)
	{
			...
			$this->db->where('orders.user_id', $user_id);
			 ...
			 if($category_name!=NULL && $category_description!=NULL)
				...
				$this->db->where("orders.created_at BETWEEN '".$category_name."' AND '".$category_description."'");
			   }
...
public function count_orders_search($category_name=NULL, $category_description=NULL, $user_id)
	{
	$this->db->select('*');
	$this->db->from('orders');
	$this->db->where('orders.user_id', $user_id);
...
public function count_total_orders($id)
	{
   $query = $this->db->query("select id, sum(total) as total_amt from orders WHERE user_id = '$id'");
	...
	public function get_orders_by_appointment_id($id = null)
	{
        if ($id != null) {
            $this->db->select('*');
            $this->db->from('orders');
	...
	public function get_orders_by_appointments($id = null)
	{
        if ($id != null) {
            $this->db->select('*');
            $this->db->from('orders');
	...
	public function get_id($id)
	{
		...
		$this->db->select('orders.*, u.first_name, u.last_name');
			$this->db->from('orders');
	...
	{
		$this->db->insert('orders', $blogData);
	...
	{
		$this->db->where('id', $id);
		$this->db->update('orders', $blogData);
	}
